...
#		return
	...
	for i in Equipment_slots.size():
		_register_item(i, Equipment_slots.list_of_equipment_limbs[i])
	inventory_list.get_child(0).grab_focus()
	show()


func _register_item(index: int, slot: Limb_Component) -> void:
	var item_button: Button = inventory_menu_item_scene.instantiate()
	var char: String = String.chr("a".unicode_at(0) + index)
	item_button.text = "( %s ) %s" % [char, slot.get_entity_name()]
