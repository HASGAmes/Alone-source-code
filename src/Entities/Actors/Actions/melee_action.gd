...
func handle_hit_chance_and_damage(attacker:FighterComponent,currentlimb:Limb_Component,defender:FighterComponent=null,tile:Tile = null)->bool:
	...
	print(attacker.entity.get_entity_name(),currentlimb.equiped_item_definition)
	if currentlimb.equiped_item_definition != null:
		attack_description = "%s attacks %s with their %s" %[attacker.entity.get_entity_name(),attackednamestring,attacker.current_primary_limb.equiped_item_definition.name]
		var dice =currentlimb.equiped_item_definition.equipment_item_component.damage_dice.duplicate()
		print(dice)
	...
	if tile!=null:
		...
		if attacker.current_primary_limb.equiped_item_definition != null:
			attack_description = "%s attacks %s with their %s" %[attacker.entity.get_entity_name(),tile.tile_name,attacker.current_primary_limb.equiped_item_definition.name]
		else:
	...
	if defender!=null:
		...
		defender.take_damage(damage,DamageTypes.DAMAGE_TYPES.BLUDGEONING,attack_description)
	return true
